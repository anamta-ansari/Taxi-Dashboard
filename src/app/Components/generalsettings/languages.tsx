...
export default function Languages() {
    ...
    //toggle RTL
    const handleToggleRTL = (id: string) => {
        const updated = languages.map(item =>
            item.id === id ? { ...item, RTL: !item.RTL } : item
        );
        localStorage.setItem('languages', JSON.stringify(updated));
        setLanguages(updated);
        window.dispatchEvent(new Event('localStorageUpdate'));
    ...
    const activeCount = languages.filter(item => item.status).length;
    const inactiveCount = languages.filter(item => !item.status).length;
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="mb-6">
                    <div className="mb-4 flex justify-between items-center">
                        <h1 className="text-[28px] font-bold text-gray-800 dark:text-gray-300">Languages</h1>
                        <Button
                ...
                <div className="bg-white rounded-lg shadow dark:bg-[#161c24]">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            <thead className="bg-gray-50 border-b dark:bg-[#161c24]">
                                <tr>
                                    <th className="px-4 py-3 text-left">
                                        <input type="checkbox" className="rounded" />
                                    </th>
                                    <th className="dark:text-gray-300 px-4 py-3 text-left text-sm font-semibold text-gray-700">Title</th>
                                    <th className="dark:text-gray-300 px-4 py-3 text-left text-sm font-semibold text-gray-700">Locale</th>
                                    <th className="dark:text-gray-300 px-4 py-3 text-left text-sm font-semibold text-gray-700">App Locale</th>
                                    <th className="dark:text-gray-300 px-4 py-3 text-left text-sm font-semibold text-gray-700">RTL</th>
                            ...
                            <tbody>
                                {languages.length === 0 ? (
                                    ...
                                    languages.map((item) => (
                                        <tr key={item.id} className="border-b 50">
                                            ...
                                            <td className="px-4 py-3 text-gray-700 dark:text-gray-300">{item.locale}</td>
                                            <td className="px-4 py-3 text-gray-700 dark:text-gray-300">{item.AppLocale}</td>
                                            {/* <td className="px-4 py-3 text-gray-700">{item.RTL}</td> */}
                                            <td className="px-4 py-3">
                                                <button
                                                    onClick={() => handleToggleRTL(item.id)}
                                                    className={`w-10 h-5 rounded-full flex items-center transition-all duration-200 ${item.RTL ? 'bg-blue-500' : 'bg-gray-400'
                                                        }`}
                                                >
                                                    <span
                                                        className={`w-4 h-4 bg-white rounded-full shadow transform transition-transform duration-200 ${item.RTL ? 'translate-x-5' : 'translate-x-1'
                                                            }`}
                                                    ></span>
                                                </button>
                                            </td>
                    ...
                    <div className="flex justify-between items-center px-4 py-3 border-t">
                        <div className="text-sm text-gray-600">
                            {languages.length} {languages.length === 1 ? 'Item' : 'Items'}
                        </div>
                    </div>
                </div>
            </div>
